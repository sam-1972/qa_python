...
def test_add_new_book_invalid_length_or_empty(collector, book_name):
    collector.add_new_book(book_name)
    assert book_name not in collector.get_books_genre()

def test_add_duplicate_book_is_ignored(collector):
...
def test_set_book_genre_failure(collector, name, genre):
...
def test_get_book_genre_returns_none_for_unset(collector):
...
def test_get_books_genre_returns_current_state(collector):
...
def test_favorites_flow_and_duplicates_protection(collector):
